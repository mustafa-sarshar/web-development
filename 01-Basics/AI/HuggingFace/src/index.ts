...
const runImage2TextModel = async (modelName: string, imgUrl: string) => {
  try {
    ...
    const img2TxtRes: ImageToTextOutput = await hfInference.imageToText({
      model: modelName,
      data: imgBlob,
    });
...
const runChatCompletion = async (
  ...
  try {
    const answer: ChatCompletionOutput = await hfInference.chatCompletion({
      model: modelName,
      messages: [
        {
          role,
...
const runTextSummarizer = async (modelName: string, inputText: string) => {
  try {
    const summary: SummarizationOutput | any = await hfInference.summarization({
      model: modelName,
      inputs: inputText,
