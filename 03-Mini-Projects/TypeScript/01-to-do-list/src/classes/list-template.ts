...
class ListTemplate {
  ...
  constructor(container: HTMLOListElement) {
    this.container = container;
  }

  render(
    task: HasGetSummary & HasTaskName & HasTaskDeadline & HasTaskCategory
  ) {
    ...
    const taskCategoryStyle =
      task.taskCategory === "normal"
        ? "bg-primary"
        : task.taskCategory === "urgent"
        ? "bg-warning"
        : "bg-danger";
    spanTaskCategoryEl.classList.add(
      "badge",
      taskCategoryStyle,
      "rounded-pill"
    );
