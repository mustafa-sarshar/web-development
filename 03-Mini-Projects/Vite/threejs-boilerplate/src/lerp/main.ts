...
function lerp(from: number, to: number, speed: number, stopThreshold: number) {
  if (Math.abs(from - to) < stopThreshold) {
    return to;
  }
  ...
  return Math.abs(from - to) < stopThreshold ? to : amount;
  */
}
...
class Pickable extends Mesh {
  ...
  update(delta: number) {
    ...
    /* 
    // to use the ThreeJS lerp function
    this.clicked
    ...
    */
    ...
    // to use custom lerp function

    if (this.clicked) {
      this.position.y = lerp(this.position.y, 1, delta * 5, 0.001);
    } else {
      this.position.y = lerp(this.position.y, 0, delta * 5, 0.001);
    }
    // this.clicked ? (this.position.y = lerp(this.position.y, 1, delta * 5, 0.001)) : (this.position.y = lerp(this.position.y, 0, delta * 5, 0.001));
    ...
    if (this.hovered) {
      this.material.color.lerp(this.colorTo, delta * 10);
      this.material.roughness = lerp(this.material.roughness, 0, delta * 10, 0.001);
      this.material.metalness = lerp(this.material.metalness, 1, delta * 10, 0.001);
    } else {
      this.material.color.lerp(this.defaultColor, delta);
      this.material.roughness = lerp(this.material.roughness, 1, delta, 0.001);
      this.material.metalness = lerp(this.material.metalness, 0, delta, 0.001);
    }
    ...
    /*    if (this.clicked) {
      this.scale.set(lerp(this.scale.x, 1.5, delta * 5, 0.001), lerp(this.scale.y, 1.5, delta * 5, 0.001), lerp(this.scale.z, 1.5, delta * 5, 0.001));
    } else {
      this.scale.set(lerp(this.scale.x, 1.0, delta, 0.001), lerp(this.scale.y, 1.0, delta, 0.001), lerp(this.scale.z, 1.0, delta, 0.001));
    } */
...
const cube = new Pickable(new BoxGeometry(), new MeshStandardMaterial({ color: 0x888888 }), new Color(0xff2200));
cube.position.set(-2, 0, 0);
scene.add(cube);
pickables.push(cube);

const pyramid = new Pickable(new TetrahedronGeometry(), new MeshStandardMaterial({ color: 0x888888 }), new Color(0x0088ff));
