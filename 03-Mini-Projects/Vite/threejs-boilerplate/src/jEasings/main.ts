...
window.addEventListener("resize", () => {
  ...
  renderer.setSize(window.innerWidth, window.innerHeight);
  render(); // this line is unnecessary if you are re-rendering within the animation loop (On-Demand rendering)
});

const controls = new OrbitControls(camera, renderer.domElement);
controls.enableDamping = true;
controls.addEventListener("change", render); // this line is unnecessary if you are re-rendering within the animation loop (On-Demand rendering)

let suzanne: THREE.Mesh, plane: THREE.Mesh;

new GLTFLoader().load("/models/suzanne_scene_p2.glb", (gltf) => {
  ...
  spotLight.target = suzanne; // the spotlight follows the suzanne object
